 ...
 *  1.0 - Filters (Excerpt More)
 */
function initiator_excerpt_more_setup() {
	global $post;
	$more = 'continue reading...';
	
	return '<a class="read-more" href="' . esc_url( get_permalink( $post->ID ) ) . '"> ( ' . esc_html( $more ) . ' )</a>';
}
add_filter( 'excerpt_more', __NAMESPACE__ . '\initiator_excerpt_more_setup'); 

/**
 *  2.0 - Filters (Excerpt Length)
 */
function initiator_excerpt_length_setup() {
	if ( ! is_admin() ) {
		return 50;
	}
}
add_filter( 'excerpt_length', __NAMESPACE__ . '\initiator_excerpt_length_setup');

/**
 *  3.0 - Filters (Archive Title)
 */
function initiator_get_the_archive_title_setup() {
	if ( is_category() ) {
...
add_filter( 'get_the_archive_title', __NAMESPACE__ . '\initiator_get_the_archive_title_setup' );